...
#include "ui_CMvMultiObjectiveContourMatching.h"
#include <QCheckBox>
#include <QDebug>
...
CMvMultiObjectiveContourMatching* CMvMultiObjectiveContourMatching::s_pCMvMultiObjectiveContourMatching = nullptr;

CMvMultiObjectiveContourMatching::CMvMultiObjectiveContourMatching(QWidget *parent)
	: QWidget(parent),
	ui(new Ui::CMvMultiObjectiveContourMatching)
{
	ui->setupUi(this);
	...
	**=================================二级菜单相关函数及信号与槽的链接==============================**
	...
	//初始化数据
	initCMvMultiObjectiveContourMatching();
	...
	**====================================参数设置信号与槽的链接=====================================**
	...
	//获取查找个数 数值
	connect(ui->spinBox_searchesNumber, SIGNAL(valueChanged(int)), this, SLOT(slotGetSearchesNumberValue(int)));
	...
	//获取比例下限 数值
	connect(ui->doubleSpinBox_minScale, SIGNAL(valueChanged(double)), this, SLOT(slotGetMinScaleValue(double)));

	//获取比例上限 数值
	connect(ui->doubleSpinBox_maxScale, SIGNAL(valueChanged(double)), this, SLOT(slotGetMaxScaleValue(double)));
...
//析构函数
CMvMultiObjectiveContourMatching::~CMvMultiObjectiveContourMatching()
{
	qDebug() << "析构函数";
...
//单例化
CMvMultiObjectiveContourMatching* CMvMultiObjectiveContourMatching::Instance()
{
	if (s_pCMvMultiObjectiveContourMatching == nullptr) {
		s_pCMvMultiObjectiveContourMatching = new CMvMultiObjectiveContourMatching;
	}
	return s_pCMvMultiObjectiveContourMatching;
}


//释放内存
void CMvMultiObjectiveContourMatching::destroy()
{
	if (s_pCMvMultiObjectiveContourMatching)
	{
		delete s_pCMvMultiObjectiveContourMatching;
		s_pCMvMultiObjectiveContourMatching = nullptr;
	}//if (s_pCMvMultiObjectiveContourMatching)
}

//初始化数据
void CMvMultiObjectiveContourMatching::initCMvMultiObjectiveContourMatching()
{
	...
	//获取匹配速度 数值
	ui->comboBox_matchingSpeed->setCurrentIndex(0);
...
\*===============================================================================================*/
//获取检测器名称
void CMvMultiObjectiveContourMatching::slotGetDetectorNameValue()
{
	if (m_signalEnable) {
...
//获取启用检测器选中信息
void CMvMultiObjectiveContourMatching::slotGetEnableDetectorValue(bool state)
{
	if (m_signalEnable) {
...
//获取ROI来源
void CMvMultiObjectiveContourMatching::slotGetROISourcesValue(int index)
{
	if (m_signalEnable) {
...
//获取自己创建选中信息
void CMvMultiObjectiveContourMatching::slotGetCreateYourselfValue(bool state)
{
	if (m_signalEnable) {
...
\*===============================================================================================*/
////初始化二级菜单
void CMvMultiObjectiveContourMatching::initMenu()
{
//	//初始化图片来源菜单
...
void CMvMultiObjectiveContourMatching::initMenuByTest()
{
	//初始化图片来源菜单
...
//根据点击位置选择弹框
void CMvMultiObjectiveContourMatching::slotClickPushButton(int row, int col)
{
	qDebug() << "位置确定";
...
//菜单动作点击
void CMvMultiObjectiveContourMatching::soltMenuTriggered(QAction* action)
{
	//设置点击栏的显示内容
...
\*===============================================================================================*/
//点击编辑轮廓
void CMvMultiObjectiveContourMatching::slotGetEditingContoursIsClick()
{
	qDebug() << "点击编辑轮廓";
}

//点击锁
void CMvMultiObjectiveContourMatching::slotGetLockIsClick()
{
	qDebug() << "点击锁";
}
...
//获取匹配速度 数值
void CMvMultiObjectiveContourMatching::slotGetMatchingSpeedValue(int index)
{
	if (m_signalEnable) {
		qDebug() << "获取匹配速度" << ui->comboBox_matchingSpeed->itemText(index);
	}
}

//获取边缘阈值
void CMvMultiObjectiveContourMatching::slotGetEdgeThresholdtValue(int index)
{
	if (m_signalEnable) {
...
//获取边缘梯度阈值 数值
void CMvMultiObjectiveContourMatching::slotGetEdgeGradientThresholdtValue(int Value)
{
	if (m_signalEnable) {
...
//获取查找个数 数值
void CMvMultiObjectiveContourMatching::slotGetSearchesNumberValue(int Value)
{
	if (m_signalEnable) {
		qDebug() << "获取查找个数" << ui->spinBox_searchesNumber->value() << "  " << Value;
	}
}

//获取边缘极性检测方式 数值
void CMvMultiObjectiveContourMatching::slotGetEdgePolarityValue(int index)
{
	if (m_signalEnable) {
...
//获取滤波尺寸 数值
void CMvMultiObjectiveContourMatching::slotGetFilterSizeValue(int index)
{
	if (m_signalEnable) {
...
//获取相邻目标最小距离 数值
void CMvMultiObjectiveContourMatching::slotGetMinimumRangeValue(int Value)
{
	if (m_signalEnable) {
...
//获取最小匹配度 数值
void CMvMultiObjectiveContourMatching::slotGetMinimumMatchingValue(double Value)
{
	if (m_signalEnable) {
...
//获取最小角度 数值
void CMvMultiObjectiveContourMatching::slotGetMinimumAngleValue(double Value)
{
	if (m_signalEnable) {
...
//获取最大角度 数值
void CMvMultiObjectiveContourMatching::slotGetMaximumAngleValue(double Value)
{
	if (m_signalEnable) {
...
//获取比例下限 数值
void CMvMultiObjectiveContourMatching::slotGetMinScaleValue(double Value)
{
	if (m_signalEnable) {
...
//获取比例上限 数值
void CMvMultiObjectiveContourMatching::slotGetMaxScaleValue(double Value)
{
	if (m_signalEnable) {
...
\*===============================================================================================*/
//获取检测区域的掩膜 数值
void CMvMultiObjectiveContourMatching::slotMaskOfDetectionAreaValue(int Index)
{
	if (m_signalEnable) {
...
//获取编辑方式 数值
void CMvMultiObjectiveContourMatching::slotEditModeValue(int Index)
{
	if (m_signalEnable) {
...
//获取画笔尺寸 数值
void CMvMultiObjectiveContourMatching::slotBrushSizeValue(int Value)
{
	if (m_signalEnable) {
...
//点击清空掩膜
void CMvMultiObjectiveContourMatching::slotEmptyMaskIsClick()
{
	if (m_signalEnable) {
		m_maskCount = 0;
...
//点击掩盖所有
void CMvMultiObjectiveContourMatching::slotCoverUpEverythingClick()
{
	if (m_signalEnable) {
		qDebug() << "掩盖所有被点了";
	}
...
//点击保存修改
void CMvMultiObjectiveContourMatching::slotSaveChangesClick()
{
	if (m_signalEnable) {
		qDebug() << "保存修改被点了";
	}
...
\*===============================================================================================*/
//获取线条宽度 数值
void CMvMultiObjectiveContourMatching::slotGetLineWidthValue(int Value)
{
	if (m_signalEnable) {
...
//获取启动绘制选中信息
void CMvMultiObjectiveContourMatching::slotGetStartUpDrawingValue(bool State)
{
	if (m_signalEnable) {
...
//获取线条颜色 数值
void CMvMultiObjectiveContourMatching::slotGetLineColourValue(int Index)
{
	if (m_signalEnable) {
...
\*===============================================================================================*/
//点击 放大
void CMvMultiObjectiveContourMatching::slotAmplifyThePictureIsClick()
{
	qDebug() << "放大被点了";
}

//点击 缩小
void CMvMultiObjectiveContourMatching::slotShrinkThePictureIsClick()
{
	qDebug() << "缩小被点了";
}

//点击 最好尺寸
void CMvMultiObjectiveContourMatching::slotBestSizeOfPictureIsClick()
{
	qDebug() << "最好尺寸被点了";
}

//点击 锁定ROI
void CMvMultiObjectiveContourMatching::slotLockROIIsClick()
{
	qDebug() << "锁定ROI被点了";
}

//点击 单次
void CMvMultiObjectiveContourMatching::slotOnceIsClick()
{
	qDebug() << "单次被点了";
}

//点击 确定
void CMvMultiObjectiveContourMatching::slotMakeSureIsClick()
{
	qDebug() << "确定被点了";
}

//点击 取消
void CMvMultiObjectiveContourMatching::slotCancelIsClick()
{
	qDebug() << "取消被点了";
}
