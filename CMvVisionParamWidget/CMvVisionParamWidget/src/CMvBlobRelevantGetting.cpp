...
#include "ui_CMvBlobRelevantGetting.h"
#include <QStringList>
...
CMvBlobRelevantGetting * CMvBlobRelevantGetting::s_pCMvBlobRelevantGetting = nullptr;

CMvBlobRelevantGetting::CMvBlobRelevantGetting(QWidget *parent)
	: QWidget(parent),
	ui(new Ui::CMvBlobRelevantGetting)
{ 
	ui->setupUi(this);
	...
	//选择获取内容
	connect(ui->comboBox_selection, SIGNAL(currentIndexChanged(int)), this, SLOT(slotSelection(int)));

	//获取Blob起始位置
	connect(ui->spinBox_startingLocation, SIGNAL(valueChanged(int)), this, SLOT(slotGetStartingLocationValue(int)));

	//获取Blob终于位置
	connect(ui->spinBox_endLocation, SIGNAL(valueChanged(int)), this, SLOT(slotGetEndLocationValue(int)));
...
CMvBlobRelevantGetting::~CMvBlobRelevantGetting()
{
	qDebug() << "析构函数";
...
//单例
CMvBlobRelevantGetting * CMvBlobRelevantGetting::Instance()
{
	if (s_pCMvBlobRelevantGetting == nullptr)
	{
		s_pCMvBlobRelevantGetting = new CMvBlobRelevantGetting;
	}
	return s_pCMvBlobRelevantGetting;

}

//释放内存
void CMvBlobRelevantGetting::destroy()
{
	if (s_pCMvBlobRelevantGetting)
	{
		delete s_pCMvBlobRelevantGetting;
		s_pCMvBlobRelevantGetting = nullptr;
	}
}

//初始化数据
void CMvBlobRelevantGetting::initCMvBlobRelevantGetting()
{
	...
	//选择获取内容
	ui->comboBox_selection->setCurrentIndex(0);

	if (ui->comboBox_selection->currentIndex() == 0)
	{
		ui->checkBox_valuTrig->setHidden(true);
	}
	else
	{
		ui->checkBox_valuTrig->setHidden(false);
	}
	
	//获取Blob起始位置
	ui->spinBox_startingLocation->setValue(0);
	
	//获取Blob终于位置
	ui->spinBox_endLocation->setValue(5000);
...
//选择获取内容
void CMvBlobRelevantGetting::slotSelection(int index)
{
	if (m_signalEnable) {
		qDebug() << "选择获取内容" << ui->comboBox_selection->itemText(index) << " " << index;
...
//获取Blob起始位置
void CMvBlobRelevantGetting::slotGetStartingLocationValue(int value)
{
	if (m_signalEnable) {
		qDebug() << "获取Blob起始位置" << ui->spinBox_startingLocation->value() << " " << value;
	}
}

//获取Blob终于位置
void CMvBlobRelevantGetting::slotGetEndLocationValue(int value)
{
	if (m_signalEnable) {
		qDebug() << "获取Blob终于位置" << ui->spinBox_endLocation->value() << " " << value;
	}
}

//初始化图片来源菜单
void CMvBlobRelevantGetting::initMenuByTest()
{
	m_pColorImageMenu = m_pSecondLevelMenu->initMenuByTest(ui->tableWidget_input, m_pColorImageMenuData);
}

void CMvBlobRelevantGetting::slotClickPushButton(int row, int col)
{
	qDebug() << "位置确定";
...
//菜单动作点击
void CMvBlobRelevantGetting::soltMenuTriggered(QAction* action)
{
	//设置点击栏的显示内容
...
\*===============================================================================================*/
//获取检测器名称
void CMvBlobRelevantGetting::slotGetDetectorNameValue()
{
	if (m_signalEnable) {
...
//获取启用检测器选中信息
void CMvBlobRelevantGetting::slotGetEnableDetectorValue(bool state)
{
	if (m_signalEnable) {
...
\*===============================================================================================*/
//获取启动绘制选中信息
void CMvBlobRelevantGetting::slotGetStartUpDrawingValue(bool State)
{
	if (m_signalEnable) {
...
\*===============================================================================================*/
//点击 放大
void CMvBlobRelevantGetting::slotAmplifyThePictureIsClick()
{
	qDebug() << "放大被点了";
}

//点击 缩小
void CMvBlobRelevantGetting::slotShrinkThePictureIsClick()
{
	qDebug() << "缩小被点了";
}

//点击 最好尺寸
void CMvBlobRelevantGetting::slotBestSizeOfPictureIsClick()
{
	qDebug() << "最好尺寸被点了";
}

//点击 锁定ROI
void CMvBlobRelevantGetting::slotLockROIIsClick()
{
	qDebug() << "锁定ROI被点了";
}

//点击 单次
void CMvBlobRelevantGetting::slotOnceIsClick()
{
	qDebug() << "单次被点了";
}

//点击 确定
void CMvBlobRelevantGetting::slotMakeSureIsClick()
{
	qDebug() << "确定被点了";
}

//点击 取消
void CMvBlobRelevantGetting::slotCancelIsClick()
{
	qDebug() << "取消被点了";
}
