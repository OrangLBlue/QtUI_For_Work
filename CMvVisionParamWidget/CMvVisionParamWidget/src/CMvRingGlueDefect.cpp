...
#include "ui_CMvRingGlueDefect.h"
#include <QCheckBox>
#include <QDebug>
...
CMvRingGlueDefect* CMvRingGlueDefect::s_pCMvRingGlueDefect = nullptr;

CMvRingGlueDefect::CMvRingGlueDefect(QWidget *parent)
	: QWidget(parent),
	ui(new Ui::CMvRingGlueDefect)
{
	ui->setupUi(this);
	...
	**====================================参数设置信号与槽的链接=====================================**
	...
	//获取环胶显示
	connect(ui->checkBox_ringGlueDisplay, SIGNAL(clicked(bool)), this, SLOT(slotGetRingGlueDisplayIsChecked(bool)));
	...
	//获取环胶最小圆度 数值
	connect(ui->doubleSpinBox_ringGlue, SIGNAL(valueChanged(double)), this, SLOT(slotGetRingGlueValue(double)));
...
//析构函数
CMvRingGlueDefect::~CMvRingGlueDefect()
{
	qDebug() << "析构函数";
...
//单例化
CMvRingGlueDefect* CMvRingGlueDefect::Instance()
{
	if (s_pCMvRingGlueDefect == nullptr) {
		s_pCMvRingGlueDefect = new CMvRingGlueDefect;
	}
	return s_pCMvRingGlueDefect;
}


//释放内存
void CMvRingGlueDefect::destroy()
{
	if (s_pCMvRingGlueDefect)
	{
		delete s_pCMvRingGlueDefect;
		s_pCMvRingGlueDefect = nullptr;
	}//if (s_pCMvRingGlueDefect)
}

//初始化数据
void CMvRingGlueDefect::initCMvRingGlueDefect()
{
	...
	//获取环胶显示
	ui->checkBox_ringGlueDisplay->setChecked(false);
	...
	//获取环胶最小圆度 数值
	ui->doubleSpinBox_ringGlue->setValue(5);
...
\*===============================================================================================*/
//获取检测器名称
void CMvRingGlueDefect::slotGetDetectorNameValue()
{
	if (m_signalEnable) {
...
//获取启用检测器选中信息
void CMvRingGlueDefect::slotGetEnableDetectorValue(bool state)
{
	if (m_signalEnable) {
...
\*===============================================================================================*/
////初始化二级菜单
void CMvRingGlueDefect::initMenu()
{

}

void CMvRingGlueDefect::initMenuByTest()
{
	//初始化图片来源菜单
...
//根据点击位置选择弹框
void CMvRingGlueDefect::slotClickPushButton(int row, int col)
{
	qDebug() << "位置确定";
...
//菜单动作点击
void CMvRingGlueDefect::soltMenuTriggered(QAction* action)
{
	//设置点击栏的显示内容
...
\*===============================================================================================*/
//获取颜色空间
void CMvRingGlueDefect::slotGetColorSpaceValue(int index)
{
	if (m_signalEnable) {
...
//点击显示颜色直方图
void CMvRingGlueDefect::slotGetUnqualifiedPiontIsClick()
{
	qDebug() << "点击显示颜色直方图";
}

//获取滤波尺寸
void CMvRingGlueDefect::slotGetFilteringSizeValue(int index)
{
	if (m_signalEnable) {
...
//获取图像显示
void CMvRingGlueDefect::slotGetImageDisplayIsChecked(bool State)
{
	qDebug() << "获取图像显示" << State << " " << ui->checkBox_imageDisplay->isChecked();
}

//获取环胶显示
void CMvRingGlueDefect::slotGetRingGlueDisplayIsChecked(bool State)
{
	qDebug() << "获取环胶显示" << State << " " << ui->checkBox_ringGlueDisplay->isChecked();
}

//获取杂点剔除面积上限 数值
void CMvRingGlueDefect::slotGetMaxAreaValue(int Value)
{
	if (m_signalEnable) {
...
//获取杂点剔除面积下限 数值
void CMvRingGlueDefect::slotGetMinAreaValue(int Value)
{
	if (m_signalEnable) {
...
//获取涂胶宽度上限 数值
void CMvRingGlueDefect::slotGetWidthMaxValue(int Value)
{
	if (m_signalEnable) {
...
//获取涂胶宽度下限 数值
void CMvRingGlueDefect::slotGetWidthMinValue(int Value)
{
	if (m_signalEnable) {
...
//获取异胶最小长度 数值
void CMvRingGlueDefect::slotGetMinimumLengthValue(double Value)
{
	if (m_signalEnable) {
...
//获取开槽数量 数值
void CMvRingGlueDefect::slotGetlSotsNumberValue(int Value)
{
	if (m_signalEnable) {
...
//获取开槽长度上限 数值
void CMvRingGlueDefect::slotGetMaxLengthValue(int Value)
{
	if (m_signalEnable) {
...
//获取开槽长度下限 数值
void CMvRingGlueDefect::slotGetMinLengthValue(int Value)
{
	if (m_signalEnable) {
...
//获取断胶最小长度 数值
void CMvRingGlueDefect::slotGetBreakdownValue(double Value)
{
	if (m_signalEnable) {
...
//获取环胶最小圆度 数值
void CMvRingGlueDefect::slotGetRingGlueValue(double Value)
{
	if (m_signalEnable) {
		qDebug() << "获取环胶最小圆度" << ui->doubleSpinBox_ringGlue->value() << "  " << Value;
	}
}

//获取孔洞最小面积 数值
void CMvRingGlueDefect::slotGetHoleValue(double Value)
{
	if (m_signalEnable) {
...
//获取启动绘制选中信息
void CMvRingGlueDefect::slotGetStartUpDrawingValue(bool State)
{
	if (m_signalEnable) {
...
\*===============================================================================================*/
//点击 放大
void CMvRingGlueDefect::slotAmplifyThePictureIsClick()
{
	qDebug() << "放大被点了";
}

//点击 缩小
void CMvRingGlueDefect::slotShrinkThePictureIsClick()
{
	qDebug() << "缩小被点了";
}

//点击 最好尺寸
void CMvRingGlueDefect::slotBestSizeOfPictureIsClick()
{
	qDebug() << "最好尺寸被点了";
}

//点击 锁定ROI
void CMvRingGlueDefect::slotLockROIIsClick()
{
	qDebug() << "锁定ROI被点了";
}

//点击 单次
void CMvRingGlueDefect::slotOnceIsClick()
{
	qDebug() << "单次被点了";
}

//点击 确定
void CMvRingGlueDefect::slotMakeSureIsClick()
{
	qDebug() << "确定被点了";
}

//点击 取消
void CMvRingGlueDefect::slotCancelIsClick()
{
	qDebug() << "取消被点了";
}
