...
#include "ui_CMvExtractFarthestPoints.h"
#include <QCheckBox>
#include <QDebug>
...
CMvExtractFarthestPoints* CMvExtractFarthestPoints::s_pCMvExtractFarthestPoints = nullptr;

CMvExtractFarthestPoints::CMvExtractFarthestPoints(QWidget *parent)
	: QWidget(parent),
	ui(new Ui::CMvExtractFarthestPoints)
{
	ui->setupUi(this);
	...
	**====================================参数设置信号与槽的链接=====================================**
	...
	//获取引脚数量下限
	connect(ui->spinBox_minNumber, SIGNAL(valueChanged(int)), this, SLOT(slotGetMinNumberValue(int)));

	//获取引脚数量上限
	connect(ui->spinBox_maxNumber, SIGNAL(valueChanged(int)), this, SLOT(slotGetMaxNumberValue(int)));
...
//析构函数
CMvExtractFarthestPoints::~CMvExtractFarthestPoints()
{
	qDebug() << "析构函数";
...
//单例化
CMvExtractFarthestPoints* CMvExtractFarthestPoints::Instance()
{
	if (s_pCMvExtractFarthestPoints == nullptr) {
		s_pCMvExtractFarthestPoints = new CMvExtractFarthestPoints;
	}
	return s_pCMvExtractFarthestPoints;
}


//释放内存
void CMvExtractFarthestPoints::destroy()
{
	if (s_pCMvExtractFarthestPoints)
	{
		delete s_pCMvExtractFarthestPoints;
		s_pCMvExtractFarthestPoints = nullptr;
	}//if (c_pUniqueShowMessage)
}

//初始化数据
void CMvExtractFarthestPoints::initCMvExtractFarthestPoints()
{
	...
	//获取引脚数量下限
	ui->spinBox_minNumber->setValue(5);
	
	//获取引脚数量上限
	ui->spinBox_maxNumber->setValue(5);
...
\*===============================================================================================*/
//获取检测器名称
void CMvExtractFarthestPoints::slotGetDetectorNameValue()
{
	if (m_signalEnable) {
...
//获取启用检测器选中信息
void CMvExtractFarthestPoints::slotGetEnableDetectorValue(bool state)
{
	if (m_signalEnable) {
...
//获取ROI来源
void CMvExtractFarthestPoints::slotGetROISourcesValue(int index)
{
	if (m_signalEnable) {
...
//获取自己创建选中信息
void CMvExtractFarthestPoints::slotGetCreateYourselfValue(bool state)
{
	if (m_signalEnable) {
...
\*===============================================================================================*/
////初始化二级菜单
void CMvExtractFarthestPoints::initMenu()
{
//	//初始化图片来源菜单
...
void CMvExtractFarthestPoints::initMenuByTest()
{
	//初始化图片来源菜单
...
//根据点击位置选择弹框
void CMvExtractFarthestPoints::slotClickPushButton(int row, int col)
{
	qDebug() << "位置确定";
...
//菜单动作点击
void CMvExtractFarthestPoints::soltMenuTriggered(QAction* action)
{
	//设置点击栏的显示内容
...
\*===============================================================================================*/
//获取梯度阈值
void CMvExtractFarthestPoints::slotGetEdgeGradientThresholdtValue(int Value)
{
	if (m_signalEnable) {
		qDebug() << "获取梯度阈值" << ui->spinBox_threshold->value() << "  " << Value;
	}
}


//开启杂点剔除
void CMvExtractFarthestPoints::slotGetNumberIsChecked(bool state)
{
	if (m_signalEnable) {
		qDebug() << "开启杂点剔除" << state;
...
//获取边缘点数下限
void CMvExtractFarthestPoints::slotGetMinNumberValue(int Value)
{
	if (m_signalEnable) {
		qDebug() << "获取边缘点数下限" << ui->spinBox_minNumber->value() << "  " << Value;
	}
}


//获取边缘点数上限
void CMvExtractFarthestPoints::slotGetMaxNumberValue(int Value)
{
	if (m_signalEnable) {
		qDebug() << "获取边缘点数上限" << ui->spinBox_maxNumber->value() << "  " << Value;
	}
}
...
\*===============================================================================================*/
//获取检测区域的掩膜 数值
void CMvExtractFarthestPoints::slotMaskOfDetectionAreaValue(int Index)
{
	if (m_signalEnable) {
...
//获取编辑方式 数值
void CMvExtractFarthestPoints::slotEditModeValue(int Index)
{
	if (m_signalEnable) {
...
//获取画笔尺寸 数值
void CMvExtractFarthestPoints::slotBrushSizeValue(int Value)
{
	if (m_signalEnable) {
...
//点击清空掩膜
void CMvExtractFarthestPoints::slotEmptyMaskIsClick()
{
	if (m_signalEnable) {
		m_maskCount = 0;
...
//点击掩盖所有
void CMvExtractFarthestPoints::slotCoverUpEverythingClick()
{
	if (m_signalEnable) {
		qDebug() << "掩盖所有被点了";
	}
...
//点击保存修改
void CMvExtractFarthestPoints::slotSaveChangesClick()
{
	if (m_signalEnable) {
		qDebug() << "保存修改被点了";
	}
...
\*===============================================================================================*/
//获取线条宽度 数值
void CMvExtractFarthestPoints::slotGetLineWidthValue(int Value)
{
	if (m_signalEnable) {
...
//获取启动绘制选中信息
void CMvExtractFarthestPoints::slotGetStartUpDrawingValue(bool State)
{
	if (m_signalEnable) {
...
//获取线条颜色 数值
void CMvExtractFarthestPoints::slotGetLineColourValue(int Index)
{
	if (m_signalEnable) {
...
\*===============================================================================================*/
//点击 放大
void CMvExtractFarthestPoints::slotAmplifyThePictureIsClick()
{
	qDebug() << "放大被点了";
}

//点击 缩小
void CMvExtractFarthestPoints::slotShrinkThePictureIsClick()
{
	qDebug() << "缩小被点了";
}

//点击 最好尺寸
void CMvExtractFarthestPoints::slotBestSizeOfPictureIsClick()
{
	qDebug() << "最好尺寸被点了";
}

//点击 锁定ROI
void CMvExtractFarthestPoints::slotLockROIIsClick()
{
	qDebug() << "锁定ROI被点了";
}

//点击 单次
void CMvExtractFarthestPoints::slotOnceIsClick()
{
	qDebug() << "单次被点了";
}

//点击 确定
void CMvExtractFarthestPoints::slotMakeSureIsClick()
{
	qDebug() << "确定被点了";
}

//点击 取消
void CMvExtractFarthestPoints::slotCancelIsClick()
{
	qDebug() << "取消被点了";
}
