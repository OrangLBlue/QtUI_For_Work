...
CMvEdgeDistance* CMvEdgeDistance::s_pCMvEdgeDistance = nullptr;

CMvEdgeDistance::CMvEdgeDistance(QWidget *parent)
	: QWidget(parent),
	ui(new Ui::CMvEdgeDistance)
{
	ui->setupUi(this);
...
//析构函数
CMvEdgeDistance::~CMvEdgeDistance()
{
	qDebug() << "析构函数";
...
//单例化
CMvEdgeDistance* CMvEdgeDistance::Instance()
{
	if (s_pCMvEdgeDistance == nullptr) {
		s_pCMvEdgeDistance = new CMvEdgeDistance;
	}
	return s_pCMvEdgeDistance;
}


//释放内存
void CMvEdgeDistance::destroy()
{
	if (s_pCMvEdgeDistance)
	{
		delete s_pCMvEdgeDistance;
		s_pCMvEdgeDistance = nullptr;
	}//if (c_pUniqueShowMessage)
}

//初始化数据
void CMvEdgeDistance::initCMvEdgeDistance()
{
...
\*===============================================================================================*/
//获取检测器名称
void CMvEdgeDistance::slotGetDetectorNameValue()
{
	if (m_signalEnable) {
...
//获取启用检测器选中信息
void CMvEdgeDistance::slotGetEnableDetectorValue(bool state)
{
	if (m_signalEnable) {
...
//获取ROI来源
void CMvEdgeDistance::slotGetROISourcesValue(int index)
{
	if (m_signalEnable) {
...
//获取自己创建选中信息
void CMvEdgeDistance::slotGetCreateYourselfValue(bool state)
{
	if (m_signalEnable) {
...
void CMvEdgeDistance::initMenuByTest()
{
	//初始化图片来源菜单
...
//根据点击位置选择弹框
void CMvEdgeDistance::slotClickPushButton(int row, int col)
{
	qDebug() << "位置确定";
...
//菜单动作点击
void CMvEdgeDistance::soltMenuTriggered(QAction* action)
{
	//设置点击栏的显示内容
...
\*===============================================================================================*/
//获取边缘极性检测方式 数值
void CMvEdgeDistance::slotGetCoordinateInputValue(int index)
{
	if (m_signalEnable) {
...
//获取搜索模式
void CMvEdgeDistance::slotGetSearchPatternValue(int index)
{
	if (m_signalEnable) {
...
//获取最小边缘点数
void CMvEdgeDistance::slotGetEdgePointsValue(int Value)
{
	if (m_signalEnable) {
...
//获取手动阈值
void CMvEdgeDistance::slotGetEdgeIsClicked(bool state)
{
	if (m_signalEnable) {
		qDebug() << "获取手动阈值" << state;
...
//获取边缘梯度下限
void CMvEdgeDistance::slotGetMinEdgeValue(int Value)
{
	if (m_signalEnable) {
...
//获取边缘梯度上限
void CMvEdgeDistance::slotGetMaxEdgeValue(int Value)
{
	if (m_signalEnable) {
...
//获取距离下限
void CMvEdgeDistance::slotGetMinDistanceValue(double Value)
{
	if (m_signalEnable) {
...
//获取距离上限
void CMvEdgeDistance::slotGetMaxDistanceValue(double Value)
{
	if (m_signalEnable) {
...
\*===============================================================================================*/
//获取检测区域的掩膜 数值
void CMvEdgeDistance::slotMaskOfDetectionAreaValue(int Index)
{
	if (m_signalEnable) {
...
//获取编辑方式 数值
void CMvEdgeDistance::slotEditModeValue(int Index)
{
	if (m_signalEnable) {
...
//获取画笔尺寸 数值
void CMvEdgeDistance::slotBrushSizeValue(int Value)
{
	if (m_signalEnable) {
...
//点击清空掩膜
void CMvEdgeDistance::slotEmptyMaskIsClick()
{
	if (m_signalEnable) {
		m_maskCount = 0;
...
//点击掩盖所有
void CMvEdgeDistance::slotCoverUpEverythingClick()
{
	if (m_signalEnable) {
		qDebug() << "掩盖所有被点了";
	}
...
//点击保存修改
void CMvEdgeDistance::slotSaveChangesClick()
{
	if (m_signalEnable) {
		qDebug() << "保存修改被点了";
	}
...
\*===============================================================================================*/
//获取线条宽度 数值
void CMvEdgeDistance::slotGetLineWidthValue(int Value)
{
	if (m_signalEnable) {
...
//获取启动绘制选中信息
void CMvEdgeDistance::slotGetStartUpDrawingValue(bool State)
{
	if (m_signalEnable) {
...
//获取线条颜色 数值
void CMvEdgeDistance::slotGetLineColourValue(int Index)
{
	if (m_signalEnable) {
...
\*===============================================================================================*/
//点击 放大
void CMvEdgeDistance::slotAmplifyThePictureIsClick()
{
	qDebug() << "放大被点了";
}

//点击 缩小
void CMvEdgeDistance::slotShrinkThePictureIsClick()
{
	qDebug() << "缩小被点了";
}

//点击 最好尺寸
void CMvEdgeDistance::slotBestSizeOfPictureIsClick()
{
	qDebug() << "最好尺寸被点了";
}

//点击 锁定ROI
void CMvEdgeDistance::slotLockROIIsClick()
{
	qDebug() << "锁定ROI被点了";
}

//点击 单次
void CMvEdgeDistance::slotOnceIsClick()
{
	qDebug() << "单次被点了";
}

//点击 确定
void CMvEdgeDistance::slotMakeSureIsClick()
{
	qDebug() << "确定被点了";
}

//点击 取消
void CMvEdgeDistance::slotCancelIsClick()
{
	qDebug() << "取消被点了";
}
