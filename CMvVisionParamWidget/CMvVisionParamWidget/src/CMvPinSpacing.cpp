...
CMvPinSpacing* CMvPinSpacing::s_pCMvPinSpacing = nullptr;

CMvPinSpacing::CMvPinSpacing(QWidget *parent)
	: QWidget(parent),
	ui(new Ui::CMvPinSpacing)
{
	ui->setupUi(this);
	...
	**====================================参数设置信号与槽的链接=====================================**
	...
	//获取引脚上最少点数
	connect(ui->spinBox_minimumNumberPoints, SIGNAL(valueChanged(int)), this, SLOT(slotGetMinimumNumberPointsValue(int)));
...
//单例化
CMvPinSpacing* CMvPinSpacing::Instance()
{
	if (s_pCMvPinSpacing == nullptr) {
		s_pCMvPinSpacing = new CMvPinSpacing;
	}
	return s_pCMvPinSpacing;
}


//释放内存
void CMvPinSpacing::destroy()
{
	if (s_pCMvPinSpacing)
	{
		delete s_pCMvPinSpacing;
		s_pCMvPinSpacing = nullptr;
	}//if (c_pUniqueShowMessage)
}

//初始化数据
void CMvPinSpacing::initCMvPinSpacing()
{
	...
	//获取引脚上最少点数
	ui->spinBox_minimumNumberPoints->setValue(5);
...
\*===============================================================================================*/
//获取检测器名称
void CMvPinSpacing::slotGetDetectorNameValue()
{
	if (m_signalEnable) {
...
//获取启用检测器选中信息
void CMvPinSpacing::slotGetEnableDetectorValue(bool state)
{
	if (m_signalEnable) {
...
//获取ROI来源
void CMvPinSpacing::slotGetROISourcesValue(int index)
{
	if (m_signalEnable) {
...
//获取自己创建选中信息
void CMvPinSpacing::slotGetCreateYourselfValue(bool state)
{
	if (m_signalEnable) {
...
void CMvPinSpacing::initMenuByTest()
{
	//初始化图片来源菜单
...
//根据点击位置选择弹框
void CMvPinSpacing::slotClickPushButton(int row, int col)
{
	qDebug() << "位置确定";
...
//菜单动作点击
void CMvPinSpacing::soltMenuTriggered(QAction* action)
{
	//设置点击栏的显示内容
...
\*===============================================================================================*/
//获取边缘极性检测方式 数值
void CMvPinSpacing::slotGetCoordinateInputValue(int index)
{
	if (m_signalEnable) {
...
//获取边缘阈值
void CMvPinSpacing::slotGetEdgeGradientThresholdtValue(int Value)
{
	if (m_signalEnable) {
		qDebug() << "获取边缘阈值" << ui->spinBox_threshold->value() << "  " << Value;
	}
}

//获取引脚数量下限
void CMvPinSpacing::slotGetMinNumberValue(int Value)
{
	if (m_signalEnable) {
		qDebug() << "获取引脚数量下限" << ui->spinBox_minNumber->value() << "  " << Value;
	}
}


//获取引脚数量上限
void CMvPinSpacing::slotGetMaxNumberValue(int Value)
{
	if (m_signalEnable) {
		qDebug() << "获取引脚数量上限" << ui->spinBox_maxNumber->value() << "  " << Value;
	}
}

//获取引脚上最少点数
void CMvPinSpacing::slotGetMinimumNumberPointsValue(int Value)
{
	if (m_signalEnable) {
		qDebug() << "获取引脚上最少点数" << ui->spinBox_minimumNumberPoints->value() << "  " << Value;
			}
}

//获取相邻点最小间距
void CMvPinSpacing::slotGetMinDistanceValue(double Value)
{
	if (m_signalEnable) {
		qDebug() << "获取相邻点最小间距" << ui->doubleSpinBox_minDistance->value() << "  " << Value;
	}
}

//获取相邻点最大间距
void CMvPinSpacing::slotGetMaxDistanceValue(double Value)
{
	if (m_signalEnable) {
		qDebug() << "获取相邻点最大间距" << ui->doubleSpinBox_maxDistance->value() << "  " << Value;
	}
}
...
\*===============================================================================================*/
//获取检测区域的掩膜 数值
void CMvPinSpacing::slotMaskOfDetectionAreaValue(int Index)
{
	if (m_signalEnable) {
...
//获取编辑方式 数值
void CMvPinSpacing::slotEditModeValue(int Index)
{
	if (m_signalEnable) {
...
//获取画笔尺寸 数值
void CMvPinSpacing::slotBrushSizeValue(int Value)
{
	if (m_signalEnable) {
...
//点击清空掩膜
void CMvPinSpacing::slotEmptyMaskIsClick()
{
	if (m_signalEnable) {
		m_maskCount = 0;
...
//点击掩盖所有
void CMvPinSpacing::slotCoverUpEverythingClick()
{
	if (m_signalEnable) {
		qDebug() << "掩盖所有被点了";
	}
...
//点击保存修改
void CMvPinSpacing::slotSaveChangesClick()
{
	if (m_signalEnable) {
		qDebug() << "保存修改被点了";
	}
...
\*===============================================================================================*/
//获取线条宽度 数值
void CMvPinSpacing::slotGetLineWidthValue(int Value)
{
	if (m_signalEnable) {
...
//获取启动绘制选中信息
void CMvPinSpacing::slotGetStartUpDrawingValue(bool State)
{
	if (m_signalEnable) {
...
//获取线条颜色 数值
void CMvPinSpacing::slotGetLineColourValue(int Index)
{
	if (m_signalEnable) {
...
\*===============================================================================================*/
//点击 放大
void CMvPinSpacing::slotAmplifyThePictureIsClick()
{
	qDebug() << "放大被点了";
}

//点击 缩小
void CMvPinSpacing::slotShrinkThePictureIsClick()
{
	qDebug() << "缩小被点了";
}

//点击 最好尺寸
void CMvPinSpacing::slotBestSizeOfPictureIsClick()
{
	qDebug() << "最好尺寸被点了";
}

//点击 锁定ROI
void CMvPinSpacing::slotLockROIIsClick()
{
	qDebug() << "锁定ROI被点了";
}

//点击 单次
void CMvPinSpacing::slotOnceIsClick()
{
	qDebug() << "单次被点了";
}

//点击 确定
void CMvPinSpacing::slotMakeSureIsClick()
{
	qDebug() << "确定被点了";
}

//点击 取消
void CMvPinSpacing::slotCancelIsClick()
{
	qDebug() << "取消被点了";
}
