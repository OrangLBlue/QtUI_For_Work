#ifndef CMV_MULTI_OBJECTIVE_CONTOUR_MATCHING
#define CMV_MULTI_OBJECTIVE_CONTOUR_MATCHING
...
namespace Ui {
	class CMvMultiObjectiveContourMatching;
}

class CMvMultiObjectiveContourMatching : public QWidget
{
    Q_OBJECT

public:
	//单例化
	static CMvMultiObjectiveContourMatching* Instance();
	//释放内存
	static void destroy();
	...
	//初始化数据
	void initCMvMultiObjectiveContourMatching();
	//void testInit();
...
private slots:
	...
	**=================================二级菜单相关槽函数声明========================================**
	...
	//获取匹配速度 数值
	void slotGetMatchingSpeedValue(int index);
	...
	//获取查找个数 数值
	void slotGetSearchesNumberValue(int Value);
...
private:
	Ui::CMvMultiObjectiveContourMatching *ui;

	static CMvMultiObjectiveContourMatching* s_pCMvMultiObjectiveContourMatching;

	CMvMultiObjectiveContourMatching(QWidget *parent = Q_NULLPTR);
	~CMvMultiObjectiveContourMatching();
